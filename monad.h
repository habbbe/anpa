 ...
 */
template <typename T, bool Emplace, typename F, typename Monad, size_t total, size_t current, typename Arg, typename... Args>
inline constexpr auto lift_inner2([[maybe_unused]] F f, Arg&& arg, Args&&... args) {
    if constexpr (current == total) {
        ...
        return arg >>= [=](auto &&v) {
            return lift_inner2<T, Emplace, F, Monad, total, current+1>(f, args..., v);
        };
    }
}
...
inline constexpr auto lift_inner(F f, Monad&& m, Monads&&... monads) {
    return lift_inner2<T, Emplace, F, std::decay_t<Monad>, sizeof...(Monads) + 1, 0>(f, m, monads...);
}
 ...
 */
template <typename F, typename Monad, typename... Monads>
inline constexpr auto lift(F f, Monad&& m, Monads&&... monads) {
    return lift_inner<bool, false>(f, m, monads...);
}
 ...
 */
template <typename F, typename Monad, typename... Monads>
inline constexpr auto lift_lazy(F f, Monad&& m, Monads&&... monads) {
    return lift_inner<bool, false>(f, m, monads...);
}
...
inline constexpr auto lift_value(Monad&& m, Monads&&... monads) {
    return lift_inner<T, true>([](){}, m, monads...);
}
 ...
 */
template <typename T, typename Monad, typename... Monads>
inline constexpr auto lift_value_lazy(Monad&& m, Monads&&... monads) {
    return lift(lazy::make_lazy_value_forward_fun<T>(), m, monads...);
}
